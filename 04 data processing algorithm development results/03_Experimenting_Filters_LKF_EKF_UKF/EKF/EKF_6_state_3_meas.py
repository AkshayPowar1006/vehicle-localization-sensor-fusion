...
class EKF:
    ...
    def update(self, meas_value, dt:float):
        ...
        def getJacobofH(self,dt):
            theta = self._x[2]  
            H = np.array([[1,0,0,-sin(theta + pi),0, 0],
                          [0,1,0,cos(theta + pi),0, 0],
                          [1,0,0,-sin(theta + pi/4.0),0, 0],
                          [0,1,0,cos(theta + pi/4.0),0, 0],
                          [1,0,0,-sin(theta - pi/4.0),0, 0],
                          [0,1,0,cos(theta - pi/4.0),0, 0]], dtype= float)
            
            return H

        theta = self._x[2]  
        
        h = np.squeeze(np.array([[self._x[0] + cos(theta + pi)],
                                 [self._x[1] + sin(theta + pi)],
                                 [self._x[0] + cos(theta + pi/4)],
                                 [self._x[1] + sin(theta + pi/4)],
                                 [self._x[0] + cos(theta - pi/4)],
                                 [self._x[1] + sin(theta - pi/4)]]),1)
...
init_state = np.array([20,50,1, 1.2,0.1,0.005]).reshape(num_states,1)
init_cov = np.eye(num_states)
u = np.array([36.364, 
              0.005]).reshape(2,1)
process_variance = np.array([[1.5,  0,    0,    0,     0,      0],
                             [0,  1.5,    0,    0,     0,      0],
                             [0,    0, 0.15,    0,     0,      0],
                             [0,    0,    0, 0.15,     0,      0],
                             [0,    0,    0,    0, 0.015,      0],
...
for step in range(total_time_steps):
    ...
    if step>0:        
        ...
        true_mean_vehicle[:,:,step] = true_mean_vehicle[:,:,step-1] +  np.vstack((time_step * true_control_input[0,:,step-1] * np.array([[cos(theta)],[sin(theta)]]) * rad_wheel,
                                                                                     time_step * true_control_input[0,:,step-1] * rad_wheel,
...
plt.plot(mean_after_prediction[4,:,updatesteps], 'co--', label = "Pred")
plt.plot(mean_after_gps_update[4,:,updatesteps], 'ro--', label = "Corr")
plt.legend(loc='upper right')
