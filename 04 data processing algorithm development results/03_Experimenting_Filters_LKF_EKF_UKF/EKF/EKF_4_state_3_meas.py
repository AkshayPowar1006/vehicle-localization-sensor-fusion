...
class EKF:
    ...
    def predict(self, dt: float) -> None:
        ...
        def getJacobofF(self,dt:float):    
            ...
            F = np.array([[1,0, self._u[0]*(-math.sin(theta))*rad_wheel*dt, self._u[0]*(-math.sin(theta))*rad_wheel*dt*dt],
                          [0, 1, self._u[0]*(math.cos(theta))*rad_wheel*dt, self._u[0]*( math.cos(theta))*rad_wheel*dt*dt],
                          [0, 0, 1, dt],
                          [0, 0, 0, 1]],dtype=float)
    ...
    def update(self, meas_value, dt:float):
        ...
        def getJacobofH(self,dt):
            theta = self._x[2]  
            H = np.array([[1,0,-math.sin(theta + math.pi), 0],
                          [0,1,math.cos(theta + math.pi), 0],
                          [1,0,-math.sin(theta + math.pi/4.0), 0],
                          [0,1,math.cos(theta + math.pi/4.0), 0],
                          [1,0,-math.sin(theta - math.pi/4.0), 0],
                          [0,1,math.cos(theta - math.pi/4.0), 0]], dtype= float)
...
for step in range(total_time_steps):
    ...
    if step>0:        
        ...
        true_mean_vehicle[:,:,step] = true_mean_vehicle[:,:,step-1] +  np.vstack((time_step * true_control_input[0,:,step-1] * np.array([[math.cos(theta)],[math.sin(theta)]]) * rad_wheel,
